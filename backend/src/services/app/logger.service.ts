...
export class LoggerService {
  ...
  constructor() {
    ...
    this.transports = process.env.DB === 'LOCAL' || process.env.LOGGER
        ...
        winston.format.printf(
          (info) => {
            ...
            if (process.env.NODE_ENV === 'production') {
              return `${metadata}${typeof info.message === 'string' ? info.message[0] === '[' ? info.message : ` ${typeof info.message === 'object' ? JSON.stringify(info.message) : info.message}` : ''} ${info.stack ? JSON.stringify(info.stack) : ''}`;
            }
            switch (info.level) {
            case 'critical':
            case 'error':
            case 'debug':
              return this.colorizer.colorize(info.level, `${metadata}${typeof info.message === 'string' ? info.message[0] === '[' ? info.message : ` ${typeof info.message === 'object' ? JSON.stringify(info.message) : info.message}` : ''} ${info.stack ? JSON.stringify(info.stack) : ''}`);
            default:
              return `${this.colorizer.colorize(info.level, metadata)}${typeof info.message === 'string' ? info.message[0] === '[' ? info.message : ` ${typeof info.message === 'object' ? JSON.stringify(info.message) : info.message}` : ''}`;
            }
          },
        ),
      ),
